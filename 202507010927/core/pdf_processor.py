...
# Initialisiere Logger für dieses Modul
...
class PDFProcessor:
    ...
    def __init__(self):
        ...
        self.variable_extractor = None  # Lazy load
    ...
    def process_document(self, doc_pair: DocumentPair, hotfolder: HotfolderConfig) -> bool:
        ...
        try:
            ...
            # Wende XML-Feld-Mappings an, bevor Exporte laufen!
            if temp_xml_path is not None and hotfolder.xml_field_mappings:
                ...
                self.xml_processor.process_xml_with_mappings(
                    ...
                    input_path=hotfolder.input_path,  # Input-Pfad übergeben
                    original_pdf_path=doc_pair.pdf_path  # NEU: Original-Pfad übergeben
                )
            ...
            # Wende Stempel an wenn konfiguriert
            if hasattr(hotfolder, 'stamp_configs') and hotfolder.stamp_configs:
                # Konvertiere Stempel-Konfigurationen
                stamp_configs = []
                ...
                if stamp_configs:
                    ...
                    if not success:
                        logger.error(f"Stempel-Anwendung fehlgeschlagen: {message}")
            ...
            if hasattr(hotfolder, 'export_configs') and hotfolder.export_configs:
                ...
                # Führe Exporte durch - übergebe auch den ursprünglichen Pfad
                export_results = self.export_processor.process_exports(
                    ...
                    ocr_zones,
                    hotfolder.xml_field_mappings,
                    original_pdf_path=doc_pair.pdf_path,  # NEU: Original-Pfad
                    input_path=hotfolder.input_path        # NEU: Input-Pfad
                )
            ...
            # Nach den Exporten: XML-Feld-Mappings anwenden, falls vorhanden
            if temp_xml_path is not None and hotfolder.xml_field_mappings:
                mappings = [FieldMapping.from_dict(m) for m in hotfolder.xml_field_mappings]
                ocr_zones = [
                    zone if isinstance(zone, dict) else zone.to_dict()
            ...
            try:
                ...
                if temp_xml_path is not None and os.path.exists(temp_xml_path):
                    if doc_pair.xml_path is not None:
    ...
    def _build_context(self, pdf_path: str, xml_path: Optional[str], 
        ...
        # OCR-Zonen
        if hasattr(self, '_zone_cache') and pdf_path in self._zone_cache:
            context.update(self._zone_cache[pdf_path])
        
        # XML-Felder
        if xml_path and os.path.exists(xml_path) and xml_field_mappings:
            try:
                import xml.etree.ElementTree as ET
                tree = ET.parse(xml_path)
                root = tree.getroot()
                
                for field in xml_field_mappings:
                    field_name = field.get('field_name', '')
                    field_value = root.findtext(field_name, '')
    ...
    def _get_output_path(self, doc_pair: DocumentPair, hotfolder: HotfolderConfig) -> str:
        ...
        if hasattr(hotfolder, 'ocr_zones') and hotfolder.ocr_zones:
            for zone in hotfolder.ocr_zones:
                # Unterstütze sowohl dict als auch Objekt (z.B. OCRZone)
                if isinstance(zone, dict):
                    page_num = zone.get('page_num')
                    zone_val = zone.get('zone')
                    name = zone.get('name')  # Name hat bereits OCR_ Präfix
                else:
                    page_num = getattr(zone, 'page_num', None)
                    zone_val = getattr(zone, 'zone', None)
                    name = getattr(zone, 'name')  # Name hat bereits OCR_ Präfix
                
                zone_key = f"{page_num}_{zone_val}"
                if page_num is not None and zone_val is not None:
                    if zone_key not in self._zone_cache:
                        zone_text = self.ocr_processor.extract_text_from_zone(
                            doc_pair.pdf_path, page_num, zone_val
                        )
                        self._zone_cache[zone_key] = zone_text
                    # Variable mit dem Zone-Namen (hat bereits OCR_ Präfix)
                    context[name] = self._zone_cache[zone_key]
    ...
    def _convert_to_pdf_a(self, pdf_path: str, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning("ocrmypdf nicht installiert. Für PDF/A Konvertierung bitte installieren: pip install ocrmypdf")
