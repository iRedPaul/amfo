...
class StampProcessor:
            ...
            with open(pdf_path, 'rb') as pdf_file:
                ...
                for page_num in range(num_pages):
                    ...
                    for stamp in active_stamps:
                        if self._should_stamp_page(stamp, page_num + 1, num_pages):
                            # Erstelle Stempel-Overlay
                            stamp_overlay = self._create_stamp_overlay(
                                page, stamp, context, page_num + 1
                            )
                            
                            if stamp_overlay:
                                # Merge Stempel mit Seite
    ...
    def _create_stamp_overlay(self, page: PyPDF2.PageObject, stamp: StampConfig,
                             context: Dict[str, Any], page_num: int) -> Optional[PyPDF2.PageObject]:
        """Erstellt ein Stempel-Overlay für eine Seite"""
        try:
            # Hole Seitengröße
            ...
            # Wende Rotation an
            if stamp.rotation != 0 or stamp.orientation != StampOrientation.HORIZONTAL:
                # Berechne effektive Rotation
                rotation = stamp.rotation
                if stamp.orientation == StampOrientation.VERTICAL:
                    rotation += 90
                elif stamp.orientation == StampOrientation.DIAGONAL:
                    rotation += 45
            ...
            # Erstelle PDF-Objekt aus Stream
            packet.seek(0)
            overlay_pdf = PyPDF2.PdfReader(packet)
            return overlay_pdf.pages[0]
            
        except Exception as e:
            logger.error(f"Fehler beim Erstellen des Stempel-Overlays: {e}")
            return None
