...
class StampPreview:
    ...
    def _draw_stamp(self, x, y, width, height, config: StampConfig):
        ...
        # Erstelle Gruppe für Rotation
        if config.rotation != 0 or config.orientation.value != "horizontal":
            # Berechne effektive Rotation
            rotation = config.rotation
            if config.orientation.value == "vertical":
                rotation += 90
            elif config.orientation.value == "diagonal":
                rotation += 45
            
            # Rotiere um Zentrum
            self._draw_rotated_stamp(center_x, center_y, width, height, rotation, config)
        else:
            # Normale Darstellung
    ...
    def _draw_rotated_stamp(self, cx, cy, width, height, rotation, config: StampConfig):
        """Zeichnet einen rotierten Stempel (vereinfacht)"""
        # Für die Vorschau zeigen wir nur ein rotiertes Rechteck
        # Die echte Rotation wird im PDF-Processor gemacht
        
        # Konvertiere zu Radians
        rad = math.radians(rotation)
        
        # Berechne die vier Ecken
        hw = width / 2
        hh = height / 2
        ...
        if config.show_background:
            self.canvas.create_polygon(
                rotated,
                fill=config.background_color,
                outline=config.border_color if config.show_border else "",
                width=config.border_width * self.scale if config.show_border else 0,
                tags="stamp"
            )
        
        # Text in der Mitte
        text = "ROTIERT" if rotation % 90 != 0 else config.stamp_lines[0]['text'] if config.stamp_lines else ""
        self.canvas.create_text(
            cx, cy,
            text=text[:20],
