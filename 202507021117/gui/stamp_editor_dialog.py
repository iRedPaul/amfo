...
class StampEditorDialog:
    ...
    def __init__(self, parent, stamp_config: Optional[StampConfig] = None, 
        ...
        if stamp_config:
            ...
            # Standard-Stempel ohne vordefinierten Namen
    ...
    def _create_basic_tab(self):
        ...
        self.name_var = tk.StringVar(value=self.stamp_config.name if self.stamp_config else "")
        ...
        # Rotation mit 4 Buttons
        ttk.Label(frame, text="Rotation:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        rotation_frame.grid(row=2, column=1, sticky=tk.W, pady=5)
        
        # 4 Rotation Buttons
        for angle in [0, 90, 180, 270]:
            btn = ttk.Button(rotation_frame, text=f"{angle}°", width=8,
                           command=lambda a=angle: self._set_rotation(a))
            btn.pack(side=tk.LEFT, padx=2)
        
        # Aktuelle Rotation anzeigen
        self.rotation_label = ttk.Label(rotation_frame, text=f"Aktuell: {int(self.rotation_var.get())}°")
        self.rotation_label.pack(side=tk.LEFT, padx=10)
        
        frame.columnconfigure(1, weight=1)
    
    def _set_rotation(self, angle):
        """Setzt die Rotation auf einen festen Wert"""
        self.rotation_var.set(angle)
        self.rotation_label.config(text=f"Aktuell: {angle}°")
        self._update_preview()
    ...
    def _create_appearance_tab(self):
        ...
        self.border_radius_var = tk.DoubleVar(value=self.stamp_config.border_radius if self.stamp_config else 0.0)
        ...
        self.background_opacity_var = tk.DoubleVar(value=self.stamp_config.background_opacity if self.stamp_config else 1.0)
        ...
        self.show_shadow_var = tk.BooleanVar(value=self.stamp_config.show_shadow if self.stamp_config else False)
    ...
    def _create_content_tab(self):
        ...
        # Automatische Aktualisierung bei Textänderung
        self.line_text_var.trace('w', self._on_text_change)
        ...
        size_spin = ttk.Spinbox(font_frame, from_=6, to=72, increment=1,
                               textvariable=self.line_size_var, width=8,
        ...
        ttk.Checkbutton(style_frame, text="Fett",
                       variable=self.line_bold_var,
                       command=self._on_style_change).pack(side=tk.LEFT, padx=5)
        
        self.line_italic_var = tk.BooleanVar()
        ttk.Checkbutton(style_frame, text="Kursiv",
                       variable=self.line_italic_var,
                       command=self._on_style_change).pack(side=tk.LEFT, padx=5)
        ...
        for text, value in [("Links", "left"), ("Zentriert", "center"), ("Rechts", "right")]:
            ttk.Radiobutton(align_frame, text=text, value=value,
                           variable=self.line_align_var,
                           command=self._on_align_change).pack(side=tk.LEFT, padx=5)
    ...
    def _on_font_change(self, *args):
        """Wird aufgerufen wenn Font-Einstellungen geändert werden"""
        if self.current_line_index is not None:
            self._update_current_line()
    
    def _on_style_change(self):
        """Wird aufgerufen wenn der Stil geändert wird"""
        if self.current_line_index is not None:
            self._update_current_line()
    
    def _on_align_change(self):
        """Wird aufgerufen wenn die Ausrichtung geändert wird"""
        if self.current_line_index is not None:
            self._update_current_line()
    ...
    def _format_line_display(self, line):
        ...
        text = line['text'][:50] if line['text'] else "[Leer]"
        if len(line['text']) > 50:
            text += "..."
    ...
    def _update_current_line(self):
        ...
        if self.current_line_index is not None:
            # Update line data
            self.stamp_lines[self.current_line_index] = {
            ...
            self.lines_listbox.delete(self.current_line_index)
            self.lines_listbox.insert(self.current_line_index, 
                                    self._format_line_display(self.stamp_lines[self.current_line_index]))
            self.lines_listbox.selection_set(self.current_line_index)
    ...
    def _build_config(self):
        ...
        return StampConfig(
            ...
            margin_x=20,  # Feste Werte, da nicht mehr anpassbar
            margin_y=20,
