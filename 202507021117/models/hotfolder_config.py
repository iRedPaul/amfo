...
class OCRZone:
    ...
    def __post_init__(self):
        ...
        if not self.name.startswith('OCR_'):
            self.name = f'OCR_{self.name}'
    ...
    def from_dict(cls, data: dict) -> 'OCRZone':
        # Name wird automatisch in __post_init__ mit OCR_ Präfix versehen
        return cls(
            name=data["name"],
...
class HotfolderConfig:
    ...
    stamp_configs: List[Dict[str, Any]] = field(default_factory=list)  # Liste der Stempel-Konfigurationen
    ...
    def from_dict(cls, data: dict) -> 'HotfolderConfig':
        ...
        return cls(
            ...
            stamp_configs=data.get("stamp_configs", []),
