...
class HotfolderDialog:
    ...
    def _create_widgets(self):
        ...
        # Beschreibung - GEÄNDERT: Entfernung der Scrollbar und Begrenzung auf 3 Zeilen
        ...
        # Toolbar für Export-Liste - GEÄNDERT: Lücke zwischen Löschen und Pfeil hinzugefügt
        ...
        # Export-Liste (TreeView) - GEÄNDERT: Spalten neu angeordnet, erste leere Spalte entfernt
        ...
        # Spalten konfigurieren - GEÄNDERT: Neue Spaltenaufteilung
        ...
        self.export_tree.column("Format", width=120)  # Mehr Platz für Format
        ...
        # Tab 4: Stempel
        self.stamp_frame = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(self.stamp_frame, text="Stempel (Beta)")
        
        # Stempel-Beschreibung
        stamp_desc = ttk.Label(self.stamp_frame, 
            text="Konfigurieren Sie Eingangsstempel, die automatisch auf PDFs angewendet werden.",
            wraplength=800, foreground="gray")
        stamp_desc.pack(anchor=tk.W, pady=(0, 10))
        
        # Stempel-Liste
        stamp_list_frame = ttk.LabelFrame(self.stamp_frame, text="Stempel", padding="10")
        stamp_list_frame.pack(fill=tk.BOTH, expand=True)
        
        # Toolbar
        stamp_toolbar = ttk.Frame(stamp_list_frame)
        stamp_toolbar.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Button(stamp_toolbar, text="+ Neuer Stempel",
                  command=self._add_stamp).pack(side=tk.LEFT, padx=2)
        ttk.Button(stamp_toolbar, text="Bearbeiten",
                  command=self._edit_stamp).pack(side=tk.LEFT, padx=2)
        ttk.Button(stamp_toolbar, text="Löschen",
                  command=self._delete_stamp).pack(side=tk.LEFT, padx=2)
        ttk.Button(stamp_toolbar, text="↑",
                  command=self._move_stamp_up).pack(side=tk.LEFT, padx=20)
        ttk.Button(stamp_toolbar, text="↓",
                  command=self._move_stamp_down).pack(side=tk.LEFT, padx=2)
        
        # Stempel-Treeview
        columns = ('Name', 'Position', 'Seiten', 'Status')
        self.stamp_tree = ttk.Treeview(stamp_list_frame, columns=columns, height=10)
        self.stamp_tree.heading('#0', text='')
        self.stamp_tree.heading('Name', text='Name')
        self.stamp_tree.heading('Position', text='Position')
        self.stamp_tree.heading('Seiten', text='Seiten')
        self.stamp_tree.heading('Status', text='Status')
        
        # Spaltenbreiten
        self.stamp_tree.column('#0', width=0, stretch=False)
        self.stamp_tree.column('Name', width=200)
        self.stamp_tree.column('Position', width=150)
        self.stamp_tree.column('Seiten', width=150)
        self.stamp_tree.column('Status', width=100)
        
        self.stamp_tree.pack(fill=tk.BOTH, expand=True)
        
        # Doppelklick zum Bearbeiten
        self.stamp_tree.bind('<Double-Button-1>', lambda e: self._edit_stamp())
    ...
    def _layout_widgets(self):
        ...
        # Beschreibung - GEÄNDERT: Vereinfachtes Layout ohne Scrollbar
        ...
        # GEÄNDERT: Korrektes Spacing für die Toolbar-Buttons
        ...
        self.delete_export_button.pack(side=tk.LEFT, padx=(0, 10))  # Mehr Abstand vor dem Separator
    ...
    def _add_export_to_tree(self, export: ExportConfig):
        ...
        # GEÄNDERT: Neue Spaltenreihenfolge - Aktiv zuerst
        aktiv = "✓" if export.enabled else "✗"
        
        # GEÄNDERT: WEBDAV wurde entfernt
        methode = {
        ...
        # GEÄNDERT: Neue Spaltenreihenfolge
        self.export_tree.insert("", "end", values=(aktiv, export.name, methode, format_name, ziel))
    
    def _load_stamps(self):
        """Lädt die Stempel in die TreeView"""
        # Lösche alte Einträge
        for item in self.stamp_tree.get_children():
            self.stamp_tree.delete(item)
        
        # Füge Stempel hinzu
        for i, stamp_dict in enumerate(self.stamp_configs):
            try:
                # Erstelle temporäre StampConfig für Anzeige
                from models.stamp_config import StampConfig
                stamp = StampConfig.from_dict(stamp_dict)
                
                position = stamp.position.value.replace('_', ' ').title()
                pages = stamp.apply_to_pages.title()
                if stamp.apply_to_pages == "custom":
                    pages = f"Seiten: {','.join(map(str, stamp.custom_pages[:3]))}"
                    if len(stamp.custom_pages) > 3:
                        pages += "..."
                
                status = "Aktiv" if stamp.enabled else "Inaktiv"
                
                self.stamp_tree.insert('', 'end', values=(
                    stamp.name,
                    position,
                    pages,
                    status
                ...
                logger.error(f"Fehler beim Laden des Stempels: {e}")
    
    def _add_stamp(self):
        """Fügt einen neuen Stempel hinzu"""
        from gui.stamp_editor_dialog import StampEditorDialog
        
        dialog = StampEditorDialog(
            self.dialog,
            ocr_zones=[zone for zone in self.ocr_zones],
            xml_fields=self.xml_field_mappings
        )
        
        result = dialog.show()
        if result:
            self.stamp_configs.append(result.to_dict())
            self._load_stamps()
    
    def _edit_stamp(self):
        """Bearbeitet den ausgewählten Stempel"""
        selection = self.stamp_tree.selection()
        if not selection:
            messagebox.showinfo("Info", "Bitte wählen Sie einen Stempel aus.")
            return
        
        index = self.stamp_tree.index(selection[0])
        
        from gui.stamp_editor_dialog import StampEditorDialog
        from models.stamp_config import StampConfig
        
        # Lade vorhandene Konfiguration
        stamp_config = StampConfig.from_dict(self.stamp_configs[index])
        ...
        result = dialog.show()
        if result:
            self.stamp_configs[index] = result.to_dict()
            self._load_stamps()
    
    def _delete_stamp(self):
        """Löscht den ausgewählten Stempel"""
        selection = self.stamp_tree.selection()
        if not selection:
            messagebox.showinfo("Info", "Bitte wählen Sie einen Stempel aus.")
            return
        
        if messagebox.askyesno("Bestätigung", "Möchten Sie den ausgewählten Stempel wirklich löschen?"):
            index = self.stamp_tree.index(selection[0])
            del self.stamp_configs[index]
            self._load_stamps()
    
    def _move_stamp_up(self):
        """Verschiebt den Stempel nach oben"""
        selection = self.stamp_tree.selection()
        if not selection:
            return
        ...
        index = self.stamp_tree.index(selection[0])
        if index > 0:
            self.stamp_configs[index], self.stamp_configs[index-1] = \
                self.stamp_configs[index-1], self.stamp_configs[index]
            self._load_stamps()
            
            # Wähle verschobenes Element wieder aus
            new_selection = self.stamp_tree.get_children()[index-1]
            self.stamp_tree.selection_set(new_selection)
    
    def _move_stamp_down(self):
        """Verschiebt den Stempel nach unten"""
        selection = self.stamp_tree.selection()
        if not selection:
            return
        
        index = self.stamp_tree.index(selection[0])
        if index < len(self.stamp_configs) - 1:
            self.stamp_configs[index], self.stamp_configs[index+1] = \
                self.stamp_configs[index+1], self.stamp_configs[index]
            self._load_stamps()
