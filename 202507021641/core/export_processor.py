...
class ExportProcessor:
    ...
    def _export_pdf_a(self, pdf_path: str, export_path: str, filename: str,
        ...
        try:
            ...
            # Entscheide basierend auf OCR-Bedarf
            if searchable and not has_text:
                # OCR ist erforderlich - verwende OCRmyPDF mit minimalen Einstellungen
                args = {
                    "output_type": "pdfa-2",
                    ...
                    "rotate_pages": True,
                    "deskew": False,  # Verhindert Transkodierung
                    "clean": False,
                ...
                # Keine OCR nötig - verwende Ghostscript direkt für PDF/A-Konvertierung
                logger.info("Verwende Ghostscript für verlustfreie PDF/A-Konvertierung")
                
                # Hole Ghostscript-Pfad aus Einstellungen
                gs_cmd = None
                try:
                    ...
                    if os.path.exists(settings_file):
                        with open(settings_file, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                            gs_path = data.get('application_paths', {}).get('ghostscript')
                            if gs_path and os.path.exists(gs_path):
                                gs_cmd = gs_path
                except Exception as e:
                    logger.debug(f"Konnte Ghostscript-Pfad nicht aus Einstellungen laden: {e}")
                
                # Fallback auf Standard-Befehle
                if not gs_cmd:
                    if os.name == 'nt':
                        # Windows
                ...
                if not gs_cmd:
                    ...
                    # Fallback: Verwende OCRmyPDF ohne OCR
                    args = {
                        "output_type": "pdfa-2",
                        "pdfa_image_compression": "lossless",
                        "optimize": 0,
                ...
                # Erstelle temporäre Datei für Ghostscript
                temp_output = output_file + '.tmp'
                
                # Ghostscript-Befehl für verlustfreie PDF/A-Konvertierung
                cmd = [
                    gs_cmd,
                    '-dBATCH',
                    '-dNOPAUSE',
                    ...
                    '-dColorImageFilter=/FlateEncode',  # Verlustfreie Komprimierung
                    '-dGrayImageFilter=/FlateEncode',   # Verlustfreie Komprimierung
                    ...
                    '-dPassThroughJPEGImages=true',  # JPEG unverändert
                    '-dCompressPages=false',  # Keine Seitenkomprimierung
                    '-dOptimize=false',  # Keine Optimierung
                ...
                try:
                    ...
                    if result.returncode == 0 and os.path.exists(temp_output):
                        # Ersetze Ausgabedatei
                        shutil.move(temp_output, output_file)
                        logger.info("Ghostscript PDF/A-Konvertierung erfolgreich")
                        return True, f"PDF/A exportiert: {os.path.basename(output_file)}"
                    else:
                    ...
                    logger.error(f"Ghostscript-Aufruf fehlgeschlagen: {gs_error}")
                    # Aufräumen
                    if os.path.exists(temp_output):
                        os.remove(temp_output)
