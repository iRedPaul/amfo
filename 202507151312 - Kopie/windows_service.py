    ...
    def main(self):
        ...
        try:
            ...
            if not valid:
                ...
                self.logger.warning("Service läuft im Demo-Modus - Hotfolder können nicht aktiviert werden")
                
                # Schreibe Event Log
                servicemanager.LogMsg(
                    servicemanager.EVENTLOG_WARNING_TYPE,
                    servicemanager.PYS_SERVICE_WARNING,
                    (f"Hotfolder PDF Processor Service: {message} - Demo-Modus aktiv",)
                )
            else:
                # Log Lizenzinfo
                license_type = license_info.get("type", "unbekannt")
                self.logger.info(f"Lizenz validiert: {license_type.upper()}")
                
                if "days_remaining" in license_info:
                    days = license_info["days_remaining"]
                    if days <= 7:
                        self.logger.warning(f"Lizenz läuft in {days} Tagen ab")
