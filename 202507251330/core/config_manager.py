...
class ConfigManager:
    ...
    def save_config(self) -> None:
        ...
        try:
            ...
            # Sende Reload-Signal an den Dienst mit Wiederholungslogik
            try:
                ...
                for i in range(max_retries):
                    ...
                    # Prüfe auf den spezifischen "Pipe-Fehler" und wiederhole den Versuch
                    if "ausgelastet" in str(response) and i < max_retries - 1:
                        logger.warning(f"Dienst ist beschäftigt, versuche erneut in 2 Sekunden... ({i+1}/{max_retries})")
                        time.sleep(2)
                    else:
                        logger.warning(f"Dienst konnte nicht benachrichtigt werden: {response}")
                        break # Bei anderen Fehlern oder nach letztem Versuch abbrechen
            except ImportError:
