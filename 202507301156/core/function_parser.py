...
try:
    from core.config_manager import get_counter_manager
except ImportError:
...
class FunctionParser:
    ...
    def __init__(self):
        ...
        # Database Processor für SQL-Abfragen
        self.db_processor = DatabaseProcessor() if DatabaseProcessor else None
    ...
    def _validate_date_format(self, format_string: str) -> bool:
        """
        Validiert einen Datumsformat-String
        
        Returns:
            bool: True wenn gültig
        """
        # Prüfe auf ungültige Kombinationen
        invalid_patterns = [
            r'(?<!m)m{5,}',  # Mehr als 4 m hintereinander
            r'(?<!d)d{5,}',  # Mehr als 4 d hintereinander
            r'(?<!y)y{5,}',  # Mehr als 4 y hintereinander
        ]
        
        for pattern in invalid_patterns:
            if re.search(pattern, format_string):
                logger.warning(f"Ungültiges Datumsformat: {format_string}")
                return False
    ...
    # NEU: Datenbank-Funktion
    def _sql(self, database_name: str, query: str, column_name: str) -> str:
        """
        SQL Funktion - Führt eine SQL-Abfrage aus und gibt einen Wert zurück
        
        Args:
            database_name: Name der konfigurierten Datenbank-Verbindung
            query: SQL-Abfrage mit Variablen in <Variable> Syntax
            column_name: Name der Spalte, deren Wert zurückgegeben werden soll
            
        Returns:
            Der Wert aus der ersten Zeile der angegebenen Spalte
        """
        try:
            if not self.db_processor:
                logger.error("DatabaseProcessor nicht verfügbar")
                return ""
            
            # Ersetze Variablen in der Query
            processed_query = self._replace_variables(query)
            
            logger.debug(f"SQL-Abfrage: {processed_query}")
            
            # Führe Abfrage aus
            result = self.db_processor.execute_query(database_name, processed_query, column_name)
            ...
            logger.error(f"Fehler bei SQL-Funktion: {e}")
            return ""
...
class VariableExtractor:
    ...
    @staticmethod
    def get_level_variables(file_path: str, input_path: str) -> Dict[str, Any]:
        """
        Extrahiert Level-Variablen basierend auf der Ordnerstruktur
        
        Args:
            file_path: Vollständiger Pfad zur Datei
            input_path: Pfad zum Hotfolder (Input-Ordner)
            
        Returns:
            Dictionary mit level0 bis level5 Variablen
        """
        from pathlib import Path
        ...
        try:
            ...
            # Level 0 ist immer der Name des Input-Ordners
            variables['level0'] = input_path_obj.name
            
            # Bestimme den relativen Pfad vom Input-Ordner zur Datei
            try:
                relative_path = file_path_obj.relative_to(input_path_obj)
                
                # Extrahiere die Ordner-Ebenen (ohne die Datei selbst)
                folder_parts = relative_path.parts[:-1]  # Alle Teile außer der Datei
                
                # Füge Level 1-5 hinzu
                for i in range(1, 6):  # Level 1 bis 5
                    if i <= len(folder_parts):
                        variables[f'level{i}'] = folder_parts[i-1]
                    else:
                        variables[f'level{i}'] = ""  # Leerer String wenn Level nicht existiert
                        
            except ValueError:
                # Datei ist nicht im Input-Ordner - alle Level außer 0 sind leer
                for i in range(1, 6):
                    variables[f'level{i}'] = ""
            
            # Debug-Ausgabe
            logger.debug(f"Level-Variablen für {file_path}:")
            for key, value in variables.items():
                if value:  # Nur nicht-leere Werte ausgeben
                    logger.debug(f"  {key}: {value}")
            
        except Exception as e:
            logger.error(f"Fehler beim Extrahieren der Level-Variablen: {e}")
            # Setze alle Level-Variablen auf leer im Fehlerfall
            for i in range(6):
                variables[f'level{i}'] = ""
