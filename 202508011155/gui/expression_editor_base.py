...
class ExpressionEditorBase:
    ...
    def _load_variables_and_functions(self):
        ...
        # NEU: Level-Variablen
        level_node = self.var_func_tree.insert(var_root, "end", text="Level", open=False, tags=("category",))
        level_vars = ["level0", "level1", "level2", "level3", "level4", "level5"]
        for var in level_vars:
            self.var_func_tree.insert(level_node, "end", text=var, tags=("variable",))
        ...
        # NEU: Datenbank
        database_node = self.var_func_tree.insert(func_root, "end", text="Datenbank", open=False, tags=("category",))
        self.var_func_tree.insert(database_node, "end", text="SQL", 
                                 tags=("function", 'SQL("<DATABASE>","<QUERY>","<COLUMN>")'))
...
- Level: Ordnerstruktur-Informationen
...
- <FilePath>: Ordnerpfad der Datei""",
                ...
                "Level": """LEVEL-VARIABLEN

Informationen über die Ordnerstruktur, in der die Datei liegt.

Diese Variablen extrahieren die Ordnerhierarchie vom Hotfolder zur Datei:
- <level0>: Name des Input-Ordners (Hotfolder)
- <level1> bis <level5>: Namen der Unterordner

Die Level-Variablen ermöglichen es, die Ordnerstruktur in Dateinamen und Pfaden zu verwenden.

Beispiel-Struktur:
Hotfolder/2024/Januar/Rechnungen/rechnung.pdf
...
Verwenden Sie nur vertrauenswürdige Skripte!""",
                ...
                "Datenbank": """DATENBANK-FUNKTIONEN

Führen Sie SQL-Abfragen auf konfigurierten Datenbanken aus.

SQL: Führt eine SQL-Abfrage aus und gibt einen Wert zurück.

WICHTIG:
- Datenbanken müssen vorher konfiguriert werden
- Verwenden Sie Extras → Datenbank-Verbindungen
- Die Abfrage sollte genau eine Zeile zurückgeben
- Nur der Wert der ersten Zeile wird verwendet"""
            },
...
Kann in Bedingungen verwendet werden.""",
                ...
                "level0": """VARIABLE: level0

Name des Hotfolder-Ordners (Input-Ordner)

Dies ist immer der oberste Ordner, der als Hotfolder konfiguriert wurde.

Beispiel: "Hotfolder"

Verwendung: <level0>

HINWEIS: Diese Variable enthält immer einen Wert.""",
                
                "level1": """VARIABLE: level1

Name des ersten Unterordners im Hotfolder

Beispiel: "2024" (für Datei in Hotfolder/2024/...)

Verwendung: <level1>

HINWEIS: Leer, wenn die Datei direkt im Hotfolder liegt.""",
                
                "level2": """VARIABLE: level2

Name des zweiten Unterordners im Hotfolder

Beispiel: "Januar" (für Datei in Hotfolder/2024/Januar/...)

Verwendung: <level2>

HINWEIS: Leer, wenn kein zweiter Unterordner existiert.""",
                
                "level3": """VARIABLE: level3

Name des dritten Unterordners im Hotfolder

Beispiel: "Rechnungen" (für Datei in Hotfolder/2024/Januar/Rechnungen/...)

Verwendung: <level3>

HINWEIS: Leer, wenn kein dritter Unterordner existiert.""",
                
                "level4": """VARIABLE: level4

Name des vierten Unterordners im Hotfolder

Verwendung: <level4>

HINWEIS: Leer, wenn kein vierter Unterordner existiert.""",
                
                "level5": """VARIABLE: level5

Name des fünften Unterordners im Hotfolder

Verwendung: <level5>

HINWEIS: Leer, wenn kein fünfter Unterordner existiert.

BEISPIEL-VERWENDUNG:
Expression: <level1>_<level2>_<FileName>
Ergebnis: "2024_Januar_Rechnung_001"

Expression: IF("<level1>","=","2024","Aktuell","Archiv")
Ergebnis: "Aktuell" wenn im 2024-Ordner""",
...
Führt eine SQL-Abfrage auf einer konfigurierten Datenbank aus.

Syntax: SQL("<DATABASE>","<QUERY>","<COLUMN>")

Parameter:
- DATABASE: Name der konfigurierten Datenbank-Verbindung
- QUERY: SQL-Abfrage mit Variablen in <Variable> Syntax
- COLUMN: Name der Spalte, deren Wert zurückgegeben werden soll

WICHTIG:
- Datenbanken müssen vorher konfiguriert werden (Extras → Datenbank-Verbindungen)
- Die Abfrage sollte genau eine Zeile zurückgeben
- Nur der Wert aus der ersten Zeile wird verwendet
- Variablen in der Query werden automatisch ersetzt

Beispiele:
SQL("MeineDatenbank","SELECT Name FROM Kunden WHERE KundenNr = '<OCR_Kundennummer>'","Name")
→ Gibt den Kundennamen für die erkannte Kundennummer zurück

SQL("Lager","SELECT Bestand FROM Artikel WHERE ArtikelNr = '<FileName>'","Bestand")
→ Verwendet den Dateinamen als Artikelnummer

SQL("Personal","SELECT Email FROM Mitarbeiter WHERE Abteilung = '<level1>'","Email")
→ Nutzt den ersten Unterordner als Abteilung

FEHLERBEHANDLUNG:
- Bei Fehlern wird ein leerer String zurückgegeben
- Prüfen Sie die Logs bei Problemen
- Testen Sie Ihre Abfragen vorher im Datenbank-Tool

SICHERHEIT:
- Verwenden Sie parametrisierte Abfragen wo möglich
- Validieren Sie Eingaben vor der Verwendung
- Beschränken Sie Datenbankrechte auf das Nötigste"""
            }
        }
