...
class ConfigManager:
    ...
    def validate_paths(self, hotfolder: HotfolderConfig) -> tuple[bool, str]:
        ...
        # Prüfe Fehler-Pfad wenn definiert
        if hotfolder.error_path:
            if not os.path.exists(hotfolder.error_path):
                try:
                    os.makedirs(hotfolder.error_path, exist_ok=True)
                except Exception as e:
                    return False, f"Fehler-Pfad konnte nicht erstellt werden: {e}"
        
        # Prüfe ob Input, Output und Error unterschiedlich sind
        input_abs = os.path.abspath(hotfolder.input_path)
        output_abs = os.path.abspath(hotfolder.output_path)
        
        if input_abs == output_abs:
            return False, "Input- und Output-Ordner dürfen nicht identisch sein"
        
        if hotfolder.error_path:
            error_abs = os.path.abspath(hotfolder.error_path)
            if error_abs == input_abs:
                return False, "Fehler- und Input-Ordner dürfen nicht identisch sein"
            if error_abs == output_abs:
                return False, "Fehler- und Output-Ordner dürfen nicht identisch sein"
