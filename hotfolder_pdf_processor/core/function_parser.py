...
class FunctionParser:
    ...
    def __init__(self):
        self.variables = {}
        self.logger = get_logger('FunctionParser')
        self.functions = {
            # String-Funktionen
            'FORMAT': self._format,
            'TRIM': self._trim,
        ...
        while True:
            ...
            if func_name in self.functions:
                ...
                try:
                    ...
                    self.logger.debug(f"Funktion ausgeführt: {func_name}({', '.join(evaluated_args)}) = {result}")
                except Exception as e:
                    self.logger.error(f"Fehler bei Funktion {func_name}: {e}")
                    result = ""
                ...
                self.logger.warning(f"Unbekannte Funktion: {func_name}")
    ...
    def _formatdate(self, format_string: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"Fehler bei FORMATDATE: {e}")
    ...
    def _autoincrement(self, counter_name: str, start_value: str = "1", step: str = "1") -> str:
        ...
        try:
            ...
            self.logger.debug(f"AUTOINCREMENT '{counter_name}': {current_value} (nächster: {current_value + step_val})")
            return str(current_value)
            
        except Exception as e:
            self.logger.error(f"Fehler bei AUTOINCREMENT: {e}")
            return start_value
    ...
    def _if(self, var: str, operator: str, value: str, 
        ...
        try:
            ...
            self.logger.error(f"Fehler bei IF: {e}")
            return false_result
    ...
    def _regexp_match(self, var: str, pattern: str, 
        ...
        try:
            ...
            self.logger.error(f"Fehler bei REGEXP.MATCH: {e}")
            return ""
    ...
    def _regexp_replace(self, var: str, pattern: str, replacement: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"Fehler bei REGEXP.REPLACE: {e}")
            return var
    ...
    def _scripting(self, script_path: str, *args) -> str:
        ...
        try:
            ...
            if not os.path.exists(script_path):
                self.logger.error(f"Script nicht gefunden: {script_path}")
                return ""
            ...
            if script_path.endswith('.bat'):
                ...
                self.logger.error(f"Unbekannter Script-Typ: {script_path}")
                return ""
            
            if result.returncode == 0:
                return result.stdout.strip()
            else:
                self.logger.error(f"Script-Fehler: {result.stderr}")
                return ""
                
        except Exception as e:
            self.logger.error(f"Fehler bei SCRIPTING: {e}")
            return ""
...
class VariableExtractor:
    ...
    def get_xml_variables(xml_path: str) -> Dict[str, Any]:
        ...
        variables = {}
        logger = get_logger('VariableExtractor')
        ...
        try:
            ...
            logger.debug(f"XML-Variablen extrahiert: {len(variables)} Variablen aus {xml_path}")
