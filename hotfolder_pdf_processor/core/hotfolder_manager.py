...
class HotfolderManager:
    ...
    def __init__(self):
        ...
        self.logger = get_logger('HotfolderManager')
    ...
    def start(self):
        ...
        with self._lock:
            ...
            enabled_hotfolders = self.config_manager.get_enabled_hotfolders()
            for hotfolder in enabled_hotfolders:
            ...
            self.logger.info(f"Hotfolder-Manager gestartet - {len(enabled_hotfolders)} aktive Hotfolder")
    ...
    def _monitor_loop(self):
        ...
        self.logger.debug("Monitor-Loop gestartet")
        ...
        while self._running:
            try:
                ...
                self.logger.error(f"Fehler im Monitor-Loop: {e}", exc_info=True)
                time.sleep(1)
        
        self.logger.debug("Monitor-Loop beendet")
    
    def _rescan_loop(self):
        """Periodischer Rescan aller Hotfolder"""
        self.logger.debug(f"Rescan-Loop gestartet (Intervall: {self._rescan_interval} Sekunden)")
    ...
    def create_hotfolder(self, name: str, input_path: str, output_path: str,
        ...
        try:
            ...
            self.logger.info(f"Hotfolder erstellt: {name} (ID: {hotfolder_id})")
            return True, hotfolder_id
            
        except Exception as e:
            self.logger.error(f"Fehler beim Erstellen des Hotfolders: {e}", exc_info=True)
    ...
    def update_hotfolder(self, hotfolder_id: str, **kwargs) -> tuple[bool, str]:
        ...
        try:
            ...
            self.logger.error(f"Fehler beim Aktualisieren: {e}", exc_info=True)
    ...
    def delete_hotfolder(self, hotfolder_id: str) -> tuple[bool, str]:
        ...
        try:
            hotfolder = self.config_manager.get_hotfolder(hotfolder_id)
            hotfolder_name = hotfolder.name if hotfolder else "Unbekannt"
            ...
            self.logger.error(f"Fehler beim Löschen: {e}", exc_info=True)
    ...
    def toggle_hotfolder(self, hotfolder_id: str, enabled: bool) -> tuple[bool, str]:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Hotfolder {action}: {hotfolder.name}")
                return True, f"Hotfolder {action}"
            else:
                return False, message
                
        except Exception as e:
            self.logger.error(f"Fehler beim Umschalten: {e}", exc_info=True)
    ...
    def set_rescan_interval(self, seconds: int):
        ...
        if seconds >= 60:  # Mindestens 1 Minute
            ...
            self.logger.info(f"Rescan-Intervall geändert: {old_interval}s -> {seconds}s")