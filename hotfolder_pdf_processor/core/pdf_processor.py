...
class PDFProcessor:
    ...
    def __init__(self):
        ...
        self.logger.debug(f"Temporärer Arbeitsordner: {self.temp_base_dir}")
    ...
    def process_document(self, doc_pair: DocumentPair, hotfolder: HotfolderConfig) -> bool:
        ...
        self.logger.info(f"Starte Verarbeitung: {doc_pair.base_name}")
        ...
        try:
            ...
            for action in hotfolder.actions:
                if action in self.supported_actions:
                    ...
                    self.logger.debug(f"Führe Aktion aus: {action.value}")
                    success = self.supported_actions[action](pdf_to_process, params)
                    if not success:
                        self.logger.error(f"Aktion {action.value} fehlgeschlagen für {os.path.basename(doc_pair.pdf_path)}")
            ...
            if hasattr(hotfolder, 'export_configs') and hotfolder.export_configs:
                ...
                # Führe Exporte durch
                self.logger.debug(f"Führe {len(hotfolder.export_configs)} Exporte durch")
                ...
                if not all_successful:
                    ...
                    self.logger.warning(f"Export-Fehler: {', '.join(failed_exports)}")
                    ...
                    self.logger.info(f"Alle Exporte erfolgreich: {len(export_results)} Exporte")
            ...
            self.logger.info(f"Erfolgreich verarbeitet: {temp_doc_pair.base_name}")
            return True
            
        except Exception as e:
            self.logger.error(f"Fehler bei der Verarbeitung von {os.path.basename(doc_pair.pdf_path)}: {e}", exc_info=True)
            ...
            try:
                ...
                self.logger.info(f"Dateien in Fehlerpfad verschoben: {error_path}")
                
            except Exception as move_error:
                self.logger.error(f"Fehler beim Verschieben in Fehlerpfad: {move_error}")
            ...
            try:
                ...
                self.logger.warning(f"Fehler beim Aufräumen des temporären Ordners: {cleanup_error}")
    ...
    def _process_xml_data(self, xml_path: str, pdf_path: str, hotfolder: HotfolderConfig) -> None:
        ...
        try:
            ...
            self.logger.error(f"Fehler beim Verarbeiten der XML-Datei: {e}")
    ...
    def _compress_pdf(self, pdf_path: str, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not self._is_ghostscript_available():
                self.logger.error("Ghostscript nicht gefunden! PDF-Komprimierung nicht möglich.")
                return False
            
            original_size = os.path.getsize(pdf_path)
            self.logger.info(f"Starte Komprimierung (Original: {original_size / 1024 / 1024:.2f} MB)")
            ...
            if success:
                ...
                self.logger.info(f"PDF komprimiert: {original_size / 1024 / 1024:.2f} MB -> {compressed_size / 1024 / 1024:.2f} MB ({reduction_percent:.1f}% Reduktion)")
                return True
            else:
                self.logger.error("Komprimierung fehlgeschlagen")
                return False
            
        except Exception as e:
            self.logger.error(f"Fehler bei der Komprimierung: {e}", exc_info=True)
            return False
    ...
    def _compress_with_ghostscript(self, pdf_path: str, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if mono_compression == 'ccitt':
                ...
                self.logger.info("JBIG2 wird versucht, Fallback zu CCITT falls nicht verfügbar")
                cmd.extend([
            ...
            # Debug-Ausgabe
            self.logger.debug(f"Komprimierungseinstellungen:")
            self.logger.debug(f"  - Farbbilder: {color_dpi} DPI, {color_compression}")
            self.logger.debug(f"  - Graustufen: {gray_dpi} DPI, {gray_compression}")
            self.logger.debug(f"  - S/W-Bilder: {mono_dpi} DPI, {mono_compression}")
            if color_compression == 'jpeg' or gray_compression == 'jpeg':
                self.logger.debug(f"  - JPEG-Qualität: {jpeg_quality}%")
            
            # Führe Ghostscript aus
            result = subprocess.run(cmd, capture_output=True, text=True)
            
            if result.returncode != 0:
                self.logger.error(f"Ghostscript-Fehler: {result.stderr}")
                return False
            ...
            if os.path.exists(temp_output):
                ...
                if os.path.getsize(temp_output) > 0:
                    ...
                    self.logger.error("Ghostscript erzeugte eine leere Datei")
                    return False
            
            return False
            
        except Exception as e:
            self.logger.error(f"Fehler bei Ghostscript-Komprimierung: {e}", exc_info=True)
            return False
    ...
    def _split_pdf(self, pdf_path: str, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Teile PDF auf: {page_count} Seiten")
            ...
            self.logger.info(f"PDF aufgeteilt in {page_count} Seiten: {split_dir}")
            return True
            
        except Exception as e:
            self.logger.error(f"Fehler beim Aufteilen: {e}", exc_info=True)
            return False
    ...
    def _perform_ocr(self, pdf_path: str, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Führe OCR aus mit ocrmypdf...")
                ocrmypdf.ocr(
                    pdf_path,
                    pdf_path,
                    language=language,
                ...
                self.logger.info(f"OCR erfolgreich durchgeführt")
                return True
                
            except ImportError:
                # Fallback: Tesseract direkt verwenden
                self.logger.warning("ocrmypdf nicht installiert, verwende Tesseract direkt...")
                ...
                try:
                    ...
                    self.logger.error("Tesseract nicht gefunden! Bitte installieren Sie Tesseract OCR.")
                    return False
                ...
                if result.returncode == 0 and os.path.exists(temp_out_path):
                    ...
                    self.logger.info("OCR erfolgreich durchgeführt")
                    return True
                else:
                    self.logger.error(f"OCR fehlgeschlagen: {result.stderr}")
                    return False
    ...
    def _convert_to_pdf_a(self, pdf_path: str, params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Konvertiere zu PDF/A...")
                ocrmypdf.ocr(
                    pdf_path,
                    pdf_path,
                ...
                self.logger.info(f"PDF/A Konvertierung erfolgreich")
                return True
                
            except ImportError:
                self.logger.warning("ocrmypdf nicht installiert. Für PDF/A Konvertierung bitte installieren: pip install ocrmypdf")
                ...
                try:
                    ...
                    shutil.move(temp_path, pdf_path)
                    self.logger.info("Basis PDF/A Metadaten hinzugefügt (für volle Konformität ocrmypdf installieren)")
                    return True
                    
                except Exception as e:
                    self.logger.error(f"Fehler bei PDF/A Konvertierung: {e}")
                    return False
                    
        except Exception as e:
            self.logger.error(f"Fehler bei PDF/A Konvertierung: {e}", exc_info=True)
            return False
                    ...
                    if os.path.isdir(dir_path):
                        ...
                        if dir_age > 86400:  # 24 Stunden
                            try:
                                ...
                                self.logger.warning(f"Fehler beim Löschen von {work_dir}: {e}")
                
                if cleaned_count > 0:
                    self.logger.info(f"Aufräumen abgeschlossen: {cleaned_count} alte temporäre Ordner gelöscht")
                    
        except Exception as e:
            self.logger.error(f"Fehler beim Aufräumen des temporären Verzeichnisses: {e}")