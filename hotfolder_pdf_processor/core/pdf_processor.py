...
class PDFProcessor:
    ...
    def process_document(self, doc_pair: DocumentPair, hotfolder: HotfolderConfig) -> bool:
        ...
        try:
            ...
            # WICHTIG: Verarbeite XML-Felder VOR der Dateinamens-Bestimmung
            evaluated_fields = {}
            if temp_doc_pair.has_xml and hotfolder.process_pairs and hotfolder.xml_field_mappings:
                ...
                # Extrahiere die evaluierten Felder für die Dateinamens-Expression
                try:
            ...
            if is_split:
                ...
                if temp_doc_pair.has_xml:
                    ...
                    # Verarbeite XML-Feld-Mappings nochmal für finale XML
                ...
                # Normal: Bestimme Output-Pfad mit evaluierten Feldern
                output_path = self._get_output_path(temp_doc_pair, hotfolder, evaluated_fields)
                ...
                if temp_doc_pair.has_xml and hotfolder.process_pairs:
                    ...
                    # Verarbeite XML-Feld-Mappings final
                    if hotfolder.xml_field_mappings:
                        mappings = [FieldMapping.from_dict(m) for m in hotfolder.xml_field_mappings]
                        ocr_zones = []
                        if hasattr(hotfolder, 'ocr_zones'):
            ...
            # Verschiebe Dateien in Fehler-Ordner wenn definiert
            if hotfolder.error_path and os.path.exists(hotfolder.error_path):
                try:
                    # Erstelle Fehler-Zeitstempel
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    
                    # Verschiebe PDF
                    if os.path.exists(temp_pdf_path):
                        error_pdf_name = f"{timestamp}_{os.path.basename(original_pdf_path)}"
                        error_pdf_path = os.path.join(hotfolder.error_path, error_pdf_name)
                        
                        # Verhindere Überschreiben im Fehler-Ordner
                        if os.path.exists(error_pdf_path):
                            base, ext = os.path.splitext(error_pdf_path)
                            counter = 1
                            while os.path.exists(f"{base}_{counter}{ext}"):
                                counter += 1
                            error_pdf_path = f"{base}_{counter}{ext}"
                        
                        shutil.move(temp_pdf_path, error_pdf_path)
                        print(f"Fehlerhafte PDF verschoben nach: {error_pdf_path}")
                    
                    # Verschiebe XML wenn vorhanden
                    if temp_xml_path and os.path.exists(temp_xml_path):
                        error_xml_name = f"{timestamp}_{os.path.basename(original_xml_path)}"
                        error_xml_path = os.path.join(hotfolder.error_path, error_xml_name)
                        
                        # Verhindere Überschreiben
                        if os.path.exists(error_xml_path):
                            base, ext = os.path.splitext(error_xml_path)
                            counter = 1
                            while os.path.exists(f"{base}_{counter}{ext}"):
                                counter += 1
                            error_xml_path = f"{base}_{counter}{ext}"
                        
                        shutil.move(temp_xml_path, error_xml_path)
                        print(f"Fehlerhafte XML verschoben nach: {error_xml_path}")
                    
                except Exception as error_move_error:
                    print(f"Fehler beim Verschieben in Fehler-Ordner: {error_move_error}")
                    # Falls Fehler-Ordner nicht funktioniert, verschiebe zurück zum Input
                    self._move_back_to_input(temp_pdf_path, original_pdf_path, 
                                           temp_xml_path, original_xml_path)
            else:
                # Kein Fehler-Ordner definiert: Verschiebe zurück zum Input
                self._move_back_to_input(temp_pdf_path, original_pdf_path, 
                                       temp_xml_path, original_xml_path)
    ...
    def _get_output_path(self, doc_pair: DocumentPair, hotfolder: HotfolderConfig, 
                        evaluated_fields: Dict[str, str] = None) -> str:
        ...
        # Füge evaluierte XML-Felder zum Kontext hinzu
        if evaluated_fields:
