...
class PDFProcessor:
    ...
    def process_document(self, doc_pair: DocumentPair, hotfolder: HotfolderConfig) -> bool:
        ...
        # Strukturiertes Logging für Verarbeitungsstart
        logger.info("Dokumentverarbeitung gestartet", extra={
            'pdf_file': os.path.basename(doc_pair.pdf_path),
            'has_xml': doc_pair.has_xml,
            'hotfolder': hotfolder.name,
            'actions': [action.value for action in hotfolder.actions]
        })
        ...
        try:
            ...
            for action in hotfolder.actions:
                if action in self.supported_actions:
                    ...
                    logger.debug(f"Führe Aktion aus: {action.value}")
            ...
            logger.info("Dokumentverarbeitung erfolgreich", extra={
                'pdf_file': os.path.basename(doc_pair.pdf_path),
                'hotfolder': hotfolder.name,
