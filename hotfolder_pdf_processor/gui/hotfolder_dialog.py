...
class HotfolderDialog:
    ...
    def _layout_widgets(self):
        ...
        # Export-Dateiname
        self.filename_label.grid(row=4, column=0, sticky=tk.W, pady=(0, 5))
        self.filename_frame.grid(row=4, column=1, sticky="we", pady=(0, 5))
        self.filename_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.filename_button.pack(side=tk.LEFT, padx=(5, 0))
        
        # Optionen
        self.options_frame.grid(row=5, column=0, columnspan=2, sticky="we", 
                               pady=(10, 5))
        self.process_pairs_check.pack(anchor=tk.W)
        self.xml_fields_button.pack(anchor=tk.W, pady=(5, 0))
        
        # OCR-Zonen
        self.ocr_zones_frame.grid(row=6, column=0, columnspan=2, sticky="we", 
                                 pady=(10, 5))
        ...
        # Aktionen
        self.actions_frame.grid(row=7, column=0, columnspan=2, sticky="we", 
                               pady=(10, 5))
    ...
    def _edit_filename_expression(self):
        ...
        dialog = ExpressionDialog(
            ...
            description="Definieren Sie den Dateinamen für die exportierten Dateien.\n"
                       "Verwenden Sie Variablen und Funktionen für dynamische Namen.",
            xml_field_mappings=self.xml_field_mappings  # WICHTIG: Übergebe XML-Feld-Mappings
        )
        result = dialog.show()
